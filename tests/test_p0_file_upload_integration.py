...
class TestFileProcessing(unittest.TestCase):
    ...
    def test_01_file_processor_text_file(self):
        ...
        filepath = self.create_test_file('test_doc.txt', 
            "这是一份测试文档\n包含多行内容\n用于验证文件处理功能")
...
"""
        ...
        print(f"✅ 成功处理 Markdown 文件，包含 {result['text_content'].count('#')} 个标题")
    
    def test_03_process_uploaded_file(self):
        """测试 process_uploaded_file 函数"""
        print("\n[TEST] process_uploaded_file 整合测试...")
        
        filepath = self.create_test_file('integration_test.txt',
            "这是一份整合测试文档\n用于验证完整的处理流程")
