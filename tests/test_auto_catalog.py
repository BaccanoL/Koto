...
def test_auto_catalog_config():
    ...
    print(f"✓ 自动归纳状态: {'启用' if scheduler.is_auto_catalog_enabled() else '禁用'}")
...
def test_manual_execution():
    ...
    if not source_dirs:
        ...
        print("⚠️  手动执行测试需要真实文件，建议使用 API /api/auto-catalog/run-now 测试")
    ...
    if os.path.exists(backup_dir):
        ...
        if manifests:
            ...
            if manifest_data.get('files'):
                ...
                print(f"    - 归纳文件存在: {sample_file.get('organized_exists', 'N/A')}")
        else:
