            ...
            try:
                ...
                if event_type == 'agent_step':
                    ...
                    if step_type == 'action':
                        ...
                        obs_text = event_data.get('observation') or event_data.get('content', '')
                        print(f"  ✅ OBSERVATION (len={len(obs_text)})")
                        # Check if observation contains CPU data
                        try:
                            obs_json = json.loads(obs_text[:200])
                            if 'usage_percent' in obs_json or 'logical_cores' in obs_json:
                                cpu_card_found = True
                                print(f"     ➜ CPU data detected: {json.dumps(obs_json, ensure_ascii=False)[:120]}")
                        except:
                            pass
                    ...
                    print(f"  ❌ ERROR: {data_json.get('data', {}).get('error', 'unknown')}")
                    return False
...
def test_2_multi_turn_state_reuse():
    ...
    if state_file.exists():
        with open(state_file, 'r', encoding='utf-8') as f:
            ...
            print(f"  Memory snapshot captured: {'✅ Yes' if turn2_has_memory else '❌ No'}")
            print(f"  CPU snapshot persisted: {'✅ Yes' if 'cpu' in sys_snapshot else '❌ No'}")
    ...
    if state_file.exists():
        with open(state_file, 'r', encoding='utf-8') as f:
            ...
            print(f"  All 3 tools captured: {'✅ Yes' if all_captured else '❌ Partial'}")
