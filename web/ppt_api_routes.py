...
def render_pptx(session_id):
    ...
    try:
        ...
        if original_path:
            ...
            filename = f"{safe_title}_edited_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pptx"
            docs_dir = os.path.join(os.path.dirname(__file__), '..', 'workspace', 'documents')
