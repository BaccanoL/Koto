...
class DocumentValidator:
    ...
    def validate_modifications(
        ...
        for mod in modifications:
            o = mod.get("original") or mod.get("原文片段") or mod.get("原文") or mod.get("原文片段")
            if o and o.strip():
        ...
        for idx, mod in enumerate(modifications):
            ...
            modified = mod.get("modified") or mod.get("修改建议") or mod.get("修改后文本") or mod.get("改为")
            ...
            if count == 0:
                ...
                if len(original) < 20:
                    issues.append(f"#{idx+1}: 原文出现 {count} 次，可能替换错误 '{original[:20]}...'")
