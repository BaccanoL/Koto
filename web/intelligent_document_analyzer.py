...
class IntelligentDocumentAnalyzer:
    ...
    # 文档类型检测
    ACADEMIC_KEYWORDS = ['摘要', 'abstract', '引言', '结论', '参考文献', '关键词', '论文', '研究']
    ...
    TASK_PATTERNS = {
        'write_abstract': {
            'keywords': ['写.*摘要', '生成.*摘要', '摘要.*写', '摘要.*生成', 'abstract'],
            'requirements': ['研究背景', '研究方法', '研究结果', '研究结论', '300.*400字', '300-400'],
            'output_type': 'generate'  # 生成新内容，不修改原文
        },
        'revise_intro': {
            'keywords': ['改.*引言', '改进.*引言', '重写.*引言', '优化.*引言', '引言.*改', '引言.*不符合'],
            'requirements': ['架构', '主体', '文章', '对应', '符合'],
            'output_type': 'generate'  # 生成新内容
        },
        'revise_conclusion': {
            'keywords': ['改.*结论', '改进.*结论', '重写.*结论', '优化.*结论', '结论.*不满意', 'overcap'],
    ...
    def _identify_target_sections(self, task_type: str, doc_structure: Dict) -> List[str]:
        ...
        if task_type == 'write_abstract':
            ...
            for idx, para in enumerate(paragraphs[:20]):
                text = para.get('text', '').lower()
                if '引言' in text or 'introduction' in text or para.get('type') == 'heading' and para.get('level') == 1:
    ...
    def _generate_intro_prompt(self, doc_text: str, structure: str, user_req: str) -> str:
        ...
        for idx, line in enumerate(lines):
            if '引言' in line or 'Introduction' in line.lower():
                intro_start = idx
            elif intro_start != -1 and ('二 ' in line or '第二' in line or '2.' in line or '2 ' in line[:3]):
                intro_end = idx
                break
        
        current_intro = '\n'.join(lines[intro_start:intro_end]) if intro_start != -1 and intro_end != -1 else "未找到明确的引言部分"
...
请直接输出改进后的引言内容，无需其他说明。"""
    ...
    def _generate_conclusion_prompt(self, doc_text: str, structure: str, user_req: str) -> str:
        ...
        for idx in range(len(lines) - 1, max(0, len(lines) - 100), -1):
            if '结论' in lines[idx] or '结语' in lines[idx] or 'Conclusion' in lines[idx].lower():
                conclusion_start = idx
                break
        
        current_conclusion = '\n'.join(lines[conclusion_start:]) if conclusion_start != -1 else "未找到明确的结论部分"
