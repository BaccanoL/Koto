...
class ContextAwarenessSystem:
    ...
    CONTEXT_TYPES = {
        'professional': {
            'name': '专业工作',
            'keywords': ['项目', '代码', '会议', '报告', '文档', '设计', '开发', '测试'],
        ...
        'learning': {
            'name': '学习研究',
            'keywords': ['教程', '笔记', '学习', '课程', '书籍', '论文', '研究', '知识'],
            'file_patterns': [r'\.pdf$', r'笔记', r'学习', r'教程'],
            'time_hints': [],
            'behavior': {
                'suggestion_frequency': 'low',
                'notification_priority_threshold': 'low',
                'focus_areas': ['knowledge_management', 'concept_extraction', 'related_content'],
    ...
    def __init__(
        self,
        db_path: str = "config/context_awareness.db",
    ...
    def detect_context(self, user_id: str = "default") -> Dict:
        ...
        if scores:
            ...
            if confidence >= 0.5:  # 置信度阈值
                self._update_current_context(user_id, best_context, confidence, indicators)
    ...
    def _collect_indicators(self, user_id: str) -> Dict:
        ...
        indicators['recent_files'] = [
            e.get('file_path', '') for e in recent_events
            if e.get('file_path')
        ]
        ...
        search_events = [
            e for e in recent_events
            if e.get('event_type') == 'file_search'
        ]
        ...
        if recent_events:
            ...
            indicators['work_duration'] = (last_event - first_event).total_seconds() / 60
    ...
    def _calculate_context_scores(self, indicators: Dict) -> Dict[str, float]:
        ...
        for context_type, config in self.CONTEXT_TYPES.items():
            ...
            max_score += 0.4
            keyword_matches = 0
            all_text = ' '.join(indicators['recent_files'] + indicators['recent_searches'])
            ...
            if context_type == 'professional':
                # 专业工作：编辑、创建文件多
                professional_ops = operation_counter.get('file_edit', 0) + operation_counter.get('file_create', 0)
                score += min(professional_ops / 10, 1.0) * 0.1
            
            elif context_type == 'learning':
                # 学习：打开、搜索多
                learning_ops = operation_counter.get('file_open', 0) + operation_counter.get('file_search', 0)
                score += min(learning_ops / 10, 1.0) * 0.1
            
            elif context_type == 'creative':
                # 创作：长时间编辑单个文件
                if indicators['work_duration'] > 30 and operation_counter.get('file_edit', 0) > 5:
                    score += 0.1
            
            elif context_type == 'organization':
                # 整理：移动、删除、重命名多
                org_ops = operation_counter.get('file_organize', 0) + operation_counter.get('file_delete', 0)
    ...
    def get_context_statistics(self, user_id: str, days: int = 30) -> Dict:
        ...
        return {
            ...
            'dominant_context': max(type_stats, key=lambda k: type_stats[k]['total_minutes']) if type_stats else None
        }
    ...
    def set_user_preference(
        self, user_id: str, context_type: str, preference_key: str, preference_value: str
    ):
        """设置用户场景偏好"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
